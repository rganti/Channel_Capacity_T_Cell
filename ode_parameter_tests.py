...
class ParameterTesting(object):
    def __init__(self, steps=8):
        ...
        self.k_8_1 = np.round(np.logspace(-4.0, -3.0, num=3), 6)
        ...
        if self.steps > 8:
            self.param_grid['k_9_1'] = [
                0.0003]  # np.round(np.logspace(-5.0, -2, num=5), 6)  # np.linspace(0.00005, 0.0003, 3)
            self.param_grid['k_9_2'] = [
                0.0003]  # np.round(np.logspace(-5.0, -3, num=5), 6)  # np.linspace(0.00005, 0.0003, 3)
            self.param_grid['k_9_3'] = np.round(np.logspace(-1.0, 1, num=5),
                                                4)  # [100 * i for i in self.param_grid['k_8_2']]
    ...
    def generate_qsub(self, self_foreign=False):
        ...
        if self_foreign:
            q.write(
                "python ~/SSC_python_modules/pysb_t_cell_network.py --cubic --steps {0} --ls_lf --lf {1}\n".format(
                    self.steps,
                    self.lf))
        else:
            q.write("python ~/SSC_python_modules/pysb_t_cell_network.py --cubic --steps {0}".format(self.steps))
        q.close()
